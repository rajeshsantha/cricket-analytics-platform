...
def render(delta_base: str) -> None:
    ...
    with tabs[0]:
        ...
        with c1:
            st.dataframe(df[["bowler", "wickets", "balls_bowled", "matches"]],
                         use_container_width=True, height=400)
        with c2:
            fig = px.bar(df, x="bowler", y="wickets", text="wickets",
    ...
    with tabs[1]:
        st.subheader("Best Bowling Average (min 50 wickets)")
        ...
        fig = px.bar(df, x="bowler", y="bowling_average", text="bowling_average",
    ...
    with tabs[2]:
        st.subheader("Best Economy Rate (min 100 overs)")
        ...
        fig = px.bar(df, x="bowler", y="economy_rate", text="economy_rate",
    ...
    with tabs[3]:
        st.subheader("Death-Over Economy (overs 16–20, T20)")
        ...
        fig = px.bar(df, x="bowler", y="death_economy", text="death_economy",
    ...
    with tabs[4]:
        st.subheader("Dot Ball Percentage (min 100 balls)")
        ...
        fig = px.bar(df, x="bowler", y="dot_ball_pct", text="dot_ball_pct",
    ...
    with tabs[5]:
        ...
        df.index += 1
        st.dataframe(
            df[["bowler", "wickets", "runs_conceded", "balls",
                "spell_start_over", "spell_end_over", "match_type", "match_id"]],
